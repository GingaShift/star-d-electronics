PULSONIX_LIBRARY_ASCII "SamacSys ECAD Model"
//15932025/881649/2.50/2/3/Connector

(asciiHeader
	(fileUnits MM)
)
(library Library_1
	(padStyleDef "s270_h180"
		(holeDiam 1.8)
		(padShape (layerNumRef 1) (padShapeType Rect)  (shapeWidth 2.700) (shapeHeight 2.700))
		(padShape (layerNumRef 16) (padShapeType Rect)  (shapeWidth 2.700) (shapeHeight 2.700))
	)
	(padStyleDef "c270_h180"
		(holeDiam 1.8)
		(padShape (layerNumRef 1) (padShapeType Ellipse)  (shapeWidth 2.700) (shapeHeight 2.700))
		(padShape (layerNumRef 16) (padShapeType Ellipse)  (shapeWidth 2.700) (shapeHeight 2.700))
	)
	(textStyleDef "Normal"
		(font
			(fontType Stroke)
			(fontFace "Helvetica")
			(fontHeight 1.27)
			(strokeWidth 0.127)
		)
	)
	(patternDef "XT30UPBF" (originalName "XT30UPBF")
		(multiLayer
			(pad (padNum 1) (padStyleRef s270_h180) (pt 0.000, 0.000) (rotation 90))
			(pad (padNum 2) (padStyleRef c270_h180) (pt 0.000, 5.000) (rotation 90))
		)
		(layerContents (layerNumRef 18)
			(attr "RefDes" "RefDes" (pt 0.000, 2.500) (textStyleRef "Normal") (isVisible True))
		)
		(layerContents (layerNumRef 28)
			(line (pt -2.6 7.6) (pt 2.6 7.6) (width 0.025))
		)
		(layerContents (layerNumRef 28)
			(line (pt 2.6 7.6) (pt 2.6 -2.6) (width 0.025))
		)
		(layerContents (layerNumRef 28)
			(line (pt 2.6 -2.6) (pt -2.6 -2.6) (width 0.025))
		)
		(layerContents (layerNumRef 28)
			(line (pt -2.6 -2.6) (pt -2.6 7.6) (width 0.025))
		)
		(layerContents (layerNumRef 18)
			(line (pt -2.6 7.6) (pt 2.6 7.6) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(line (pt 2.6 7.6) (pt 2.6 -2.6) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(line (pt 2.6 -2.6) (pt -2.6 -2.6) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(line (pt -2.6 -2.6) (pt -2.6 7.6) (width 0.2))
		)
		(layerContents (layerNumRef Courtyard_Top)
			(line (pt -3.6 8.6) (pt 3.6 8.6) (width 0.1))
		)
		(layerContents (layerNumRef Courtyard_Top)
			(line (pt 3.6 8.6) (pt 3.6 -3.6) (width 0.1))
		)
		(layerContents (layerNumRef Courtyard_Top)
			(line (pt 3.6 -3.6) (pt -3.6 -3.6) (width 0.1))
		)
		(layerContents (layerNumRef Courtyard_Top)
			(line (pt -3.6 -3.6) (pt -3.6 8.6) (width 0.1))
		)
		(layerContents (layerNumRef 18)
			(line (pt 3.2 0) (pt 3.2 0) (width 0.1))
		)
		(layerContents (layerNumRef 18)
			(arc (pt 3.15, 0) (radius 0.05) (startAngle .0) (sweepAngle 180.0) (width 0.1))
		)
		(layerContents (layerNumRef 18)
			(line (pt 3.1 0) (pt 3.1 0) (width 0.1))
		)
		(layerContents (layerNumRef 18)
			(arc (pt 3.15, 0) (radius 0.05) (startAngle 180) (sweepAngle 180.0) (width 0.1))
		)
	)
	(symbolDef "XT30UPB-F" (originalName "XT30UPB-F")

		(pin (pinNum 1) (pt 0 mils -100 mils) (rotation 0) (pinLength 200 mils) (pinDisplay (dispPinName true)) (pinName (text (pt 230 mils -125 mils) (rotation 0]) (justify "Left") (textStyleRef "Normal"))
		))
		(pin (pinNum 2) (pt 0 mils 0 mils) (rotation 0) (pinLength 200 mils) (pinDisplay (dispPinName true)) (pinName (text (pt 230 mils -25 mils) (rotation 0]) (justify "Left") (textStyleRef "Normal"))
		))
		(line (pt 200 mils 100 mils) (pt 600 mils 100 mils) (width 6 mils))
		(line (pt 600 mils 100 mils) (pt 600 mils -200 mils) (width 6 mils))
		(line (pt 600 mils -200 mils) (pt 200 mils -200 mils) (width 6 mils))
		(line (pt 200 mils -200 mils) (pt 200 mils 100 mils) (width 6 mils))
		(attr "RefDes" "RefDes" (pt 650 mils 300 mils) (justify Left) (isVisible True) (textStyleRef "Normal"))
		(attr "Type" "Type" (pt 650 mils 200 mils) (justify Left) (isVisible True) (textStyleRef "Normal"))

	)
	(compDef "XT30UPB-F" (originalName "XT30UPB-F") (compHeader (numPins 2) (numParts 1) (refDesPrefix J)
		)
		(compPin "1" (pinName "1") (partNum 1) (symPinNum 1) (gateEq 0) (pinEq 0) (pinType Unknown))
		(compPin "2" (pinName "2") (partNum 1) (symPinNum 2) (gateEq 0) (pinEq 0) (pinType Unknown))
		(attachedSymbol (partNum 1) (altType Normal) (symbolName "XT30UPB-F"))
		(attachedPattern (patternNum 1) (patternName "XT30UPBF")
			(numPads 2)
			(padPinMap
				(padNum 1) (compPinRef "1")
				(padNum 2) (compPinRef "2")
			)
		)
		(attr "Manufacturer_Name" "Amass")
		(attr "Manufacturer_Part_Number" "XT30UPB-F")
		(attr "Mouser Part Number" "")
		(attr "Mouser Price/Stock" "")
		(attr "Arrow Part Number" "")
		(attr "Arrow Price/Stock" "")
		(attr "Description" "Socket; DC supply; XT30; female; PIN: 2; on PCBs; THT; Colour: yellow")
		(attr "<Hyperlink>" "https://www.tme.eu/Document/81f11ca924696756331f21b972812074/XT30PB-F.pdf")
		(attr "<Component Height>" "9.4")
		(attr "<STEP Filename>" "XT30UPB-F.stp")
		(attr "<STEP Offsets>" "X=0;Y=2.5;Z=3.41")
		(attr "<STEP Rotation>" "X=0;Y=0;Z=90")
	)

)
